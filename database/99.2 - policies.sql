...
for select using (auth.uid() = id or is_under_my_watch(id) or is_watching_me(id) or is_admin());
...
for select using (auth.uid() = id or is_under_my_watch(id) or is_admin());
...
for select using (auth.uid() = filler or is_under_my_watch(filler) or is_admin());